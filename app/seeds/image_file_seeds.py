from app.models import db, ImageFile, environment, SCHEMA
...
def seed_image_files():
    new_song_images = []

    for i in range(20):
        new_image = ImageFile(
            song_id = i,
            image_file= image_arr[i],


        )
        new_song_images.append(new_image)

    db.session.add_all(new_song_images)
    db.session.commit()


def undo_image_files():
    if environment == "production":
        db.session.execute(f"TRUNCATE table {SCHEMA}.image_files RESTART IDENTITY CASCADE;")
    else:
        db.session.execute(text("DELETE FROM image_files"))
