import boto3
import botocore
import os
import uuid



BUCKET_NAME = os.environ.get("S3_BUCKET")
S3_LOCATION = f"http://{BUCKET_NAME}.s3.amazonaws.com/"

BUCKET_NAME_2 = os.environ.get("S3_BUCKET_AUDIO")
S3_LOCATION_2 = f"http://{BUCKET_NAME_2}.s3.amazonaws.com/"

ALLOWED_EXTENSIONS = {"pdf", "png", "jpg", "jpeg", "gif","mp3"}
ALLOWED_AUDIO_EXTENSIONS = {"mp3"}

s3_images = boto3.client(
   "s3",
   aws_access_key_id=os.environ.get("S3_KEY"),
   aws_secret_access_key=os.environ.get("S3_SECRET")
)

s3_audio = boto3.client(
   "s3",
   aws_access_key_id=os.environ.get("S3_KEY_AUDIO"),
   aws_secret_access_key=os.environ.get("S3_SECRET_AUDIO")
)


def get_unique_filename(filename):
    ext = filename.rsplit(".", 1)[1].lower()
    unique_filename = uuid.uuid4().hex
    return f"{unique_filename}.{ext}"




#UPLOAD TO IMAGE BUCKET
def upload_file_to_s3_images(file, acl="public-read"):
    try:
        s3_images.upload_fileobj(
            file,
            BUCKET_NAME,
            file.filename,
            ExtraArgs={
                "ACL": acl,
                "ContentType": file.content_type
            }
        )

    except Exception as e:
        # in case the your s3 upload fails
        return {"errors": str(e)}

    return {"url": f"{S3_LOCATION}{file.filename}"}

#DELETE FROM IMAGE BUCKET
def remove_file_from_s3_images(image_url):
    # AWS needs the image file name, not the URL,
    # so you split that out of the URL
    key = image_url.rsplit("/", 1)[1]
    try:
        s3_images.delete_object(
        Bucket=BUCKET_NAME,
        Key=key
        )
    except Exception as e:
        return { "errors": str(e) }
    return True


#UPLOAD TO AUDIO BUCKET
def upload_file_to_s3_audio(file, acl="public-read"):
    try:
        s3_audio.upload_fileobj(
            file,
            BUCKET_NAME_2,
            file.filename,
            ExtraArgs={
                "ACL": acl,
                "ContentType": file.content_type
            }
        )

    except Exception as e:
        # in case the your s3 upload fails
        return {"errors": str(e)}

    return {"url": f"{S3_LOCATION_2}{file.filename}"}

#DELETE FROM AUDIO BUCKET
def remove_file_from_s3(image_url):
    # AWS needs the image file name, not the URL,
    # so you split that out of the URL
    key = image_url.rsplit("/", 1)[1]
    try:
        s3_audio.delete_object(
        Bucket=BUCKET_NAME_2,
        Key=key
        )
    except Exception as e:
        return { "errors": str(e) }
    return True
